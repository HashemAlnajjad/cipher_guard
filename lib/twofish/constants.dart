import 'dart:math';
import 'dart:typed_data';


/// This class calculates and contains all constants used in encrypting and decrypting data, including padding blocks.
class Constants {
  // Static variables
  static bool _initialized = false;

  // Static constructor
  static void initialize() {
    if (!_initialized) {
      _initializeMDS();
      _initializePaddingBlocks();
      _initialized = true;
    }
  }

  static const int ROUNDS = 16; // how many rounds of encryption (default = 16)
  static const int BLOCK_SIZE = 16; // how many bytes in one data-block
  static const int ROUND_SUBKEYS = 4 + BLOCK_SIZE ~/ 4; //number of subkeys used to whiten plain text
  static const int SK_STEP = 0x02020202; //a fixed constant used to generate even subkeys
  static const int SK_BUMP = 0x01010101; //a fixed constant used to generate odd subkeys
  static const int SK_ROTL = 9; //fixed number determining bit shift in keys generator

  /// Fixed 8x8 permutation substitution box
  static const List<List<int>> P = [
    // p0
    [
      ,
    ],
    // p1
    [
      ,
    ],
  ];

  /// Define the fixed p0/p1 permutations used in keyed S-box lookup.
  /// By changing the following constant definitions, the S-boxes will automatically get changed in the Twofish engine.
  static const int P_00 = 1;
  static const int P_01 = 0;
  static const int P_02 = 0;
  static const int P_03 = P_01 ^ 1;
  static const int P_04 = 1;
  static const int P_10 = 0;
  static const int P_11 = 0;
  static const int P_12 = 1;
  static const int P_13 = P_11 ^ 1;
  static const int P_14 = 0;
  static const int P_20 = 1;
  static const int P_21 = 1;
  static const int P_22 = 0;
  static const int P_23 = P_21 ^ 1;
  static const int P_24 = 0;
  static const int P_30 = 0;
  static const int P_31 = 1;
  static const int P_32 = 1;
  static const int P_33 = P_31 ^ 1;
  static const int P_34 = 1;

  /// Primitive polynomial for GF(256)
  static const int GF256_FDBK_2 = 0x169 ~/ 2;
  static const int GF256_FDBK_4 = 0x169 ~/ 4;

  /// Maximum Distance Separable matrix
  static final List<List<int>> MDS = List.generate(4, (_) => List.filled(256, 0)); // blank final
  static const int RS_GF_FDBK = 0x14D; // field generator

  /// Precompute the MDS matrix
  static void _initializeMDS() {
    List<int> m1 = List.filled(2, 0);
    List<int> mX = List.filled(2, 0);
    List<int> mY = List.filled(2, 0);
    for (int i = 0; i < 256; i++) {
      int j = P[0][i] & 0xFF; // compute all the matrix elements
      m1[0] = j;
      mX[0] = _Mx_X(j) & 0xFF;
      mY[0] = _Mx_Y(j) & 0xFF;

      j = P[1][i] & 0xFF;
      m1[1] = j;
      mX[1] = _Mx_X(j) & 0xFF;
      mY[1] = _Mx_Y(j) & 0xFF;

      MDS[0][i] = m1[P_00] | // fill matrix with computed elements
      mX[P_00] << 8 |
      mY[P_00] << 16 |
      mY[P_00] << 24;
      MDS[1][i] = mY[P_10] |
      mY[P_10] << 8 |
      mX[P_10] << 16 |
      m1[P_10] << 24;
      MDS[2][i] = mX[P_20] |
      mY[P_20] << 8 |
      m1[P_20] << 16 |
      mY[P_20] << 24;
      MDS[3][i] = mX[P_30] |
      m1[P_30] << 8 |
      mY[P_30] << 16 |
      mX[P_30] << 24;
    }
  }

  /// Linear feedback shift register
  /// @param x input bit which is a linear function of its previous state
  /// @return output stream from shift register used in pseudo-random generators
  static int _LFSR1(int x) {
    return (x >> 1) ^ ((x & 0x01) != 0 ? GF256_FDBK_2 : 0);
  }

  /// Linear feedback shift register
  /// @param x input bit which is a linear function of its previous state
  /// @return output stream from shift register used in pseudo-random generators
  static int _LFSR2(int x) {
    return (x >> 2) ^
    ((x & 0x02) != 0 ? GF256_FDBK_2 : 0) ^
    ((x & 0x01) != 0 ? GF256_FDBK_4 : 0);
  }

  /// x XOR with linear feedback shift register
  /// @param x input for XOR operation
  /// @return x XORed with linear feedback shift register
  static int _Mx_X(int x) {
    return x ^ _LFSR2(x);
  }

  /// x XOR with linear feedback shift register
  /// @param x input for double XOR operation
  /// @return x XORed with two linear feedback shift registers
  static int _Mx_Y(int x) {
    return x ^ _LFSR1(x) ^ _LFSR2(x);
  }

  /// Padding blocks. Static code below used to initialize those padding blocks with random data to make known-plaintext attacks difficult.
  static final Uint8List PADDING_BLOCK1 = Uint8List(16);
  static final Uint8List PADDING_BLOCK2 = Uint8List(16);

  static void _initializePaddingBlocks() {
    Random random = Random();
    PADDING_BLOCK1[0] = 128;
    for (int i = 1; i < PADDING_BLOCK1.length; i++) {
      PADDING_BLOCK1[i] = random.nextInt(61) + 2;
    }
    PADDING_BLOCK2[15] = 1;
    for (int i = 0; i < PADDING_BLOCK2.length - 1; i++) {
      PADDING_BLOCK2[i] = random.nextInt(61) + 2;
    }
  }
}


